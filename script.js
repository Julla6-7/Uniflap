...
function update(currentTime) {
    ...
    // Constant for scaling (since your original game was tuned for ~60 FPS)
    const FPS_ADJUSTMENT = 60;
    
    // START OF GAME LOGIC (Moved inside the function)
    requestAnimationFrame(update);
    if (gameOver) {
        return;
    }
    ...
    // LOGIC IS CORRECT: velocityY += gravity * timeScale * FPS_ADJUSTMENT;
    velocityY += gravity * timeScale * FPS_ADJUSTMENT;
    
    // LOGIC IS CORRECT: bird.y = Math.max(bird.y + velocityY * timeScale * FPS_ADJUSTMENT, 0);
